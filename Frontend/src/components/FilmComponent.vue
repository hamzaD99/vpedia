  ...
  <v-container>
    ...
    <v-row v-if="!filmLoading && film" align="center" justify="center">
      ...
      <v-col class="text-center" cols="12">
        <router-link :to="`/series/${film.Series.slug}`" class="d-flex justify-center" style="color: rgb(var(--v-theme-primary));">
          <h3 style="width: fit-content;">{{ $t("From") }}: {{ $i18n.locale === 'ar' ? film ? film.Series.name_arabic : '' : film ? film.Series.name_english : '' }}</h3>
        </router-link>
      </v-col>
      <v-col cols="8">
      ...
      <v-col cols="8" class="mt-3 d-flex flex-column" style="font-size: 18px;">
        ...
        <span v-if="film.reviewer_arabic || film.reviewer_english" class="mb-4">{{ $i18n.locale === 'ar' ? film.reviewer_arabic : film.reviewer_english }}</span>
        <span v-if="film.Categories.length" class="mb-2" style="color: rgb(var(--v-theme-primary));font-weight: bold;">{{ $t('Main Category') }}:</span>
        ...
        <span v-if="film.SubCategories.length" class="mb-2" style="color: rgb(var(--v-theme-primary));font-weight: bold">{{ $t('Sub Categories') }}:</span>
        <div v-if="film.SubCategories.length" class="d-flex mb-4" style="column-gap: 18px;">
          <div style="padding: 10px;background: gainsboro;color: black;border-radius: 5px;" v-for="subCategory in film.SubCategories" :key="subCategory.UUID">{{ subCategory.SubCategory ? ($i18n.locale === 'ar' ? subCategory.SubCategory.name_arabic : subCategory.SubCategory.name_english) : '' }}</div>
        </div>
      </v-col>
    </v-row>
  </v-container>
